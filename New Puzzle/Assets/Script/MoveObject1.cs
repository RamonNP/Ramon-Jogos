...
public class MoveObject1 : MonoBehaviour
{
    ...
    void Start()
    {
        //Debug.Log(this.GetComponent<Transform>().localPosition.x);
            ...
            switch (touch.phase)
            {
                ...
                case TouchPhase.Moved:
                    ...
                    if (GetComponent<Collider2D>() == Physics2D.OverlapPoint(touchPos))
                    {
                        ...
                        {
                            gameController.lockKK = 1;
                        } else if (gameController.lockKK == 1)
                        {
    ...
    IEnumerator waithMoveEnum()
    {
        ...
        else if (xcurrent <= xDest)
        {
            while (xcurrent <= xDest)
            {
                xcurrent = xcurrent + 0.1f;
                yield return new WaitForSecondsRealtime(0.01f);
                //Debug.Log(xcurrent + "-" + xDest+"--"+ transform.position.x);
                transform.position = new Vector3(xcurrent, ycurrent, z);
            }
